...
type SCCFunctions interface {
	...
	CheckCommitReadiness(chname, ccname string, cd *ChaincodeDefinition, publicState ReadWritableState, orgStates []OpaqueState) (map[string]bool, map[string][]string, error)
	...
	// and the stored data.
	QueryOrgApprovals(name string, cd *ChaincodeDefinition, orgStates []OpaqueState) (map[string]bool, map[string][]string, error)
...
func (i *Invocation) CheckCommitReadiness(input *lb.CheckCommitReadinessArgs) (proto.Message, error) {
	...
	mismatchesProto := make(map[string]*lb.CheckCommitReadinessResult_Mismatch)
	for org, items := range mismatches {
		mismatchesProto[org] = &lb.CheckCommitReadinessResult_Mismatch{Items: items}
	}
